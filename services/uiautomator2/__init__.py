...
DEBUG = False
WAIT_FOR_DEVICE_TIMEOUT = int(os.getenv("WAIT_FOR_DEVICE_TIMEOUT", 30))
...
def _get_atx_resp_json(res):
    ...
    try:
        if jsondata.get('data'):
            jsondata = gzip.decompress(base64.b64decode(jsondata.get('data'))).decode('utf-8')
...
def _get_atx_resp_text(res):
    try:
        jsondata = res.json()
        if jsondata and jsondata.get('data'):
            return gzip.decompress(base64.b64decode(jsondata.get('data'))).decode('utf-8')
    except:
        pass
...
def _get_atx_resp_raw(res):
    try:
        jsondata = res.json()
        if jsondata and jsondata.get('data'):
            return gzip.decompress(base64.b64decode(jsondata.get('data')))
    except:
        pass
...
class _Service(object):
    ...
    def _raise_for_status(self, res: requests.Response):
        if res.status_code != 200:
            if res.headers['content-type'].startswith("application/json"):
                raise RuntimeError(_get_atx_resp_json(res)["description"])
    ...
    def running(self) -> bool:
        ...
        self._raise_for_status(res)
        return _get_atx_resp_json(res).get("running")
...
class _BaseClient(object):
    ...
    def path2url(self, path: str) -> str:
        ...
        agent_url: str = self._get_atx_agent_url()

        # FIXME: 判断agent_url里是否包含?，那么直接拼接，恶心的方法，回头再优化
        if agent_url.find('?') >= 0:
            return agent_url + "&rpc=" + urlparse.quote_plus(path)

        return urlparse.urljoin(agent_url, path)
    ...
    def _prepare_atx_agent(self):
        ...
        try:
            version = requests.get(version_url, timeout=30).text
    ...
    def _wait_for_device(self, timeout=None) -> adbutils.AdbDevice:
        ...
        """
        if not timeout:
            timeout = WAIT_FOR_DEVICE_TIMEOUT if _is_tmq_production() else 15.0
    ...
    def shell(self, cmdargs: Union[str, List[str]], stream=False, timeout=60):
        ...
        if ret.status_code != 200:
            raise RuntimeError(
                ...
                ret.status_code, _get_atx_resp_text(ret))
        resp = _get_atx_resp_json(ret)
    ...
    @property
    def info(self):
        return self.jsonrpc.deviceInfo(http_timeout=30)

    @property
    def wlan_ip(self):
        ip = _get_atx_resp_text(self.http.get("/wlan/ip")).strip()
    ...
    def _jsonrpc_call(self, method, params=[], http_timeout=60):
        ...
        if res.status_code == 410:  # http status gone: session broken
            raise SessionBrokenError("app quit or crash", _get_atx_resp_text(res))
        if res.status_code != 200:
            raise BaseError(data, res.status_code, _get_atx_resp_text(res),
                            "HTTP Return code is not 200", _get_atx_resp_text(res))
        jsondata = _get_atx_resp_json(res)
    ...
    def _get_agent_version(self) -> Optional[str]:
        ...
        try:
            ...
            if r.status_code != 200:
                return None
            return _get_atx_resp_text(r).strip()
    ...
    def _is_alive(self):
        try:
            r = self.http.post("/jsonrpc/0", timeout=30, retry=False, data=json.dumps({
                "jsonrpc": "2.0",
                "id": 1,
                "method": "deviceInfo",
            }))
            if r.status_code != 200:
                return False
            if _get_atx_resp_json(r).get("error"):
                return False
            return True
    ...
    def push(self, src, dst: str, mode=0o644, show_progress=False):
        ...
        try:
            ...
            if r.status_code == 200:
                return _get_atx_resp_json(r)
            raise IOError("push", "%s -> %s" % (src, dst), _get_atx_resp_text(r))
        finally:
            fileobj.close()
    ...
    def pull(self, src: str, dst: str):
        ...
        if r.status_code != 200:
            raise FileNotFoundError("pull", src, _get_atx_resp_text(r))
...
class _Device(_BaseClient):
    ...
    def set_new_command_timeout(self, timeout: int):
        ...
        data = _get_atx_resp_json(r)
        assert data['success'], data['description']
        logger.info("%s", data['description'])

    @cached_property
    def device_info(self):
        return _get_atx_resp_json(self.http.get("/info"))

    def window_size(self):
        """ return (width, height) """
        info = _get_atx_resp_json(self.http.get('/info'))
    ...
    def screenshot(self, filename: Optional[str] = None, format="pillow"):
        ...
        """
        r = self.http.get("/screenshot/0", timeout=30)
        if filename:
            with open(filename, 'wb') as f:
                f.write(_get_atx_resp_raw(r))  # r.content
            return filename
        elif format == 'pillow':
            buff = io.BytesIO(_get_atx_resp_raw(r))  # r.content
            try:
            ...
            import cv2
            import numpy as np
            nparr = np.fromstring(_get_atx_resp_raw(r), np.uint8)  # .content
            return cv2.imdecode(nparr, cv2.IMREAD_COLOR)
        elif format == 'raw':
            return _get_atx_resp_raw(r)  # .content
        else:
...
class _AppMixIn:
    def _pidof_app(self, package_name):
        ...
        """
        text = _get_atx_resp_text(self.http.get('/pidof/' + package_name))
        if text.isdigit():
            return int(text)
    ...
    def wait_activity(self, activity, timeout=30):
        """ wait activity
        Args:
    ...
    def app_icon(self, package_name: str):
        ...
        resp.raise_for_status()
        return Image.open(io.BytesIO(_get_atx_resp_raw(resp)))  # .content
    ...
    def session(self, package_name=None, attach=False, launch_timeout=None, strict=False):
        ...
        if not attach:
            ...
            if resp.status_code == 410:  # Gone
                raise SessionBrokenError(package_name, _get_atx_resp_text(resp))
            resp.raise_for_status()
            jsondata = _get_atx_resp_json(resp)
